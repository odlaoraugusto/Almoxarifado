 ...
 *
 * NÃO usar `display: none` aqui (bug real, 2026-09-10): com
 * `display:none` o elemento sai da árvore de renderização, e em vários
 * motores de navegador o `<use>` em outro lugar do DOM não consegue
 * mais pintar o conteúdo referenciado — os ícones somem por completo,
 * sobrando só o fundo/quadrado colorido ao redor. Posição absoluta com
 * tamanho zero esconde visualmente sem tirar da árvore.
 *
 * `<symbol viewBox="0 0 24 24">`, NÃO `<g>` (segundo bug real,
 * 2026-09-10): cada `<svg className="ic">` que consome via `<use>` tem
 * um tamanho CSS diferente (14px, 15px, 24px, 26px…) e nenhum viewBox
 * próprio. `<g>` não carrega viewport/escala nenhuma pro `<use>` — as
 * coordenadas do desenho (0 a 24) eram pintadas 1 unidade = 1px DENTRO
 * de uma caixa bem menor, cortando a maior parte do ícone, um recorte
 * diferente em cada tamanho. `<symbol>` foi feito exatamente pra isso:
 * o `viewBox` dele escala o conteúdo pra caber no tamanho real do
 * `<use>`, não importa o CSS aplicado em cada lugar. */
