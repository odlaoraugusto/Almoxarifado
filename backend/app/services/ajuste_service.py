...
class AjusteService:
    ...
    obrigatório e registrado na trilha de auditoria — mesmo padrão já
    resolvido no projeto irmão (farmácia)."""

    def __init__(self):
        self.lote_repository = LoteRepository()
        self.movimentacao_repository = MovimentacaoRepository()

    def ajustar(self, db: Session, usuario: UsuarioMe, dados: AjusteCreate) -> Movimentacao:
