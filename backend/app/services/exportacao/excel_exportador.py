...
com o cabeçalho institucional mesclado nas primeiras linhas, antes da
tabela de dados."""
...
_COR_CABECALHO = "61358C"  # roxo institucional (paleta FESFSUS)
...
def _valor_seguro(valor):
    """Mitigação de 'CSV/Excel injection': texto livre digitado por
    qualquer pessoa (nome do solicitante, observação, motivo de
    substituição) que comece com =, +, - ou @ seria interpretado como
    fórmula ao abrir no Excel. Prefixar com `'` é o próprio escape nativo
    do Excel para "isto é texto literal" — não aparece na célula
    renderizada."""
...
class ExcelExportador:
    ...
    def _escrever_letterhead(
        ...
        linha = self._linha_mesclada(
            ...
            linha,
            num_colunas,
            f"Gerado em {formatar_data_hora(metadados.gerado_em)} · {metadados.gerado_por}",
            9,
            negrito=False,
            italico=True,
        )
