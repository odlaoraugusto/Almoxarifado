...
desenhado via callback `onFirstPage`/`onLaterPages` do
`SimpleDocTemplate`, repetido automaticamente em toda página, com a
tabela de dados abaixo. Página em paisagem para acomodar relatórios com
mais colunas (ex. auditoria de movimentações)."""
...
_COR_CABECALHO_TABELA = colors.HexColor("#61358c")  # roxo institucional (paleta FESFSUS)
...
class PdfExportador:
    ...
    def _montar_tabela(self, tabela: TabelaRelatorio, largura_pagina: float) -> Table:
        ...
        # Sem isso, texto livre digitado por qualquer pessoa no formulário
        # público (nome do solicitante, observação, motivo de substituição)
        # podia quebrar ou deformar o PDF de qualquer relatório que
        # incluísse aquele dado.
        cabecalho = [
            Paragraph(escape(coluna), self._estilo_cabecalho_coluna) for coluna in tabela.colunas
        ]
        linhas = [
            [
    ...
    def _desenhar_cabecalho(canvas, documento, metadados: RelatorioMetadados) -> None:
        ...
        linha_meta = f"Gerado em {formatar_data_hora(metadados.gerado_em)}  ·  {metadados.gerado_por}"
